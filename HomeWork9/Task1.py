...
async def first_step(update, context):
    ...
    if user_answer == '1':
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Неверное число, повторите ввод 1 или 2")
    ...
    while not is_win(my_game_list, count):
        ...
        if number in "1 2 3 4 5 6 7 8 9":
            ...
            if my_game_list[number-1] == number:
                ...
                await context.bot.send_message(chat_id=update.effective_chat.id, text="Это место уже занято! Введите другое число от 1 до 9. Выход - 0")
                return PLAY
        elif number == '0':
            await context.bot.send_message(chat_id=update.effective_chat.id,
                                           text="До свидания!")
            ...
            await context.bot.send_message(chat_id=update.effective_chat.id,
                                           text="Неверное значение! Введите число от 1 до 9. Выход - 0")
            return PLAY
