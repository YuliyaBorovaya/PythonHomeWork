...
async def start(update, context):
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id,
                                   text=f"Добрый день, {update.effective_user.first_name}! Запустить калькулятор?")
    await context.bot.send_message(chat_id=update.effective_chat.id,
                                   text="1 - да\n2 - нет")
    return FIRST
...
async def what_format(update, context):
    ...
    if user_answer == '1':
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="С каким форматом чисел будем работать?")
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="1 - рациональное \n2 - комплексное \nq - выход")
        return START_CALC
    elif user_answer == '2':
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Хорошо, тогда до свидания!")
        return ConversationHandler.END 
    else:
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Введено неверное число, введите верное число")
...
async def start_calc(update, context):    
    ...
    if user_answer == '1':
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Калькулятор для рациональных чисел")
        number_format = 1
        await context.bot.send_message(chat_id=update.effective_chat.id,
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Введено неверное число, введите верное число или q")   
...
async def calc_compl_1d (update, context):
    ...
    numb_1 = eval(value)
    await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Введите мнимую часть первого числа")
    return CALC_COM_1M
...
async def calc_compl_1m (update, context):
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Введите действительную часть второго числа")
    return CALC_COM_2D
...
async def calc_compl_2d (update, context):
    ...
    numb_2 = eval(value)
    await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Введите мнимую часть второго числа")
    return CALC_COM_2M
    ...
    if number_format == 1:
        while not menu_ration_ex(value):
            await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Введено неверное число!")
            await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Выберите математическую операцию для рационального числа")
            return USE_CALC
    else: 
        while not menu_compl_ex(value):
            await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Введено неверное число!")
            await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Выберите математическую операцию для комплексного числа")
            return USE_CALC
    if value == 'q':
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="До свидания!")
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Возврат к предыдущему меню")
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="С каким форматом чисел будем работать")
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="1 - рациональное \n2 - комплексное \nq - выход")
